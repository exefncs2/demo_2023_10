...
with DAG(
    ...
    description='從 GCS 轉移 JSON 檔案到 PostgreSQL',
    ...
    def get_data_task(table_id, dataset_id, **kwargs):
        ...
        replace_data = downloaded_data[0][0]
    ...
    def output_db_exec(**kwargs):
        ...
        for index, table in enumerate(table_list):
            ...
            if  str(table['tableId']).split("_")[0] == "res" :
                print("test insert~")  
            else:
                continue
            
            # 使用 get_data_{index} 任務的 task_id 來拉取結果
            downloaded_data = get_data_task(table['tableId'], dataset, **kwargs) #json.loads(get_data_task(table['tableId'], dataset, **kwargs))
            print( downloaded_data ," /n <<<<<<<<<<<<< download_data")
            
            random_uuid = uuid.uuid4()
            query = f"INSERT INTO `{omop_dataset}.{json_origin_table}` (insert_id, data, create_time) VALUES ('{random_uuid}', '{downloaded_data}', '{current_timestamp}');"
            print(query)
            
            # BigQuery 任務：插入資料到 BigQuery
            insert_task = BigQueryInsertJobOperator(
                    task_id=f'insert_json_{index}',
                    # dataset_id=omop_dataset,
                    # table_id=json_origin_table,
                    configuration={
                    "query": {
                        "query": query,
                        "useLegacySql": True,
    ...
    create_empty_table = BigQueryCreateEmptyTableOperator(
                    task_id=f'create_empty_table',
                    dataset_id=omop_dataset,
                    table_id="p_origin",
                    schema_fields=[
                        {"name": "insert_id", "type": "STRING", "mode": "NULLABLE"},
                        {"name": "data", "type": "JSON", "mode": "NULLABLE"},
                        {"name": "create_time", "type": "DATE", "mode": "NULLABLE"},
                    ],
                    gcp_conn_id=google_cloud_storage_conn,
                    dag=dag,
                )
    ...
    # 定義 DAG 的任務之間的依賴性
    get_dataset_tables >> create_empty_table >> output_db
